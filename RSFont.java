...
public class RSFont extends DrawingArea {
	...
	public void drawStringMoveY(String string, int drawX, int drawY, int color,
		...
		if (string != null) {
			...
			for (int index = 0; index < string.length(); index++) {
				yOffset[index] = (int) (Math.sin((double) index / 1.5
						+ (double) randomMod) * d);
			}
	...
	public void drawCenteredStringMoveXY(String string, int drawX, int drawY,
		...
		if (string != null) {
			...
			for (int index = 0; index < string.length(); index++) {
				xMods[index] = (int) (Math.sin((double) index / 5.0
						+ (double) randomMod / 5.0) * 5.0);
				yMods[index] = (int) (Math.sin((double) index / 3.0
						+ (double) randomMod / 5.0) * 5.0);
			}
			drawBaseStringMoveXY(string, drawX - getTextWidth(string) / 2,
					drawY, xMods, yMods);
	...
	public void drawCenteredStringMoveY(String class100, int drawX, int drawY,
		...
		if (class100 != null) {
			...
			for (int index = 0; index < class100.length(); index++) {
				yOffset[index] = (int) (Math.sin((double) index / 2.0
						+ (double) i_54_ / 5.0) * 5.0);
			}
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							...
							try {
								...
								int iconModY = icon.myHeight + icon.yDrawOffset
										+ 1;
								if (transparency == 256) {
									icon.drawSprite(drawX, (drawY
											+ baseCharacterHeight - iconModY));
					...
					if (strikethroughColor != -1) {
						DrawingArea
								.drawHorizontalLine(
										drawX,
										drawY
												+ (int) ((double) baseCharacterHeight * 0.69999999999999996D),
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (character == 60) {
				...
				if (startIndex == -1) {
					...
					if (strikethroughColor != -1) {
						DrawingArea.drawHorizontalLine(drawX, drawY
								+ (int) ((double) baseCharacterHeight * 0.7),
